  ...
  <v-row justify="center" align="center">
    <v-col cols="12" sm="8" md="6">
      <v-card class="api-card" v-for="api in apis" :key="api.id" elevation="2" outlined tile>
        ...
        <v-card-text>
          ...
          <span v-if="api.status === 'W'" style="color: green">Working</span>
          <span v-else style="color: red">Failure</span>
          <br />
        </v-card-text>
      </v-card>

      <button @click="addAPI" id="plus-button">
    ...
    <v-dialog v-model="addEditDialog.active" max-width="600">
      <v-card>
        ...
        <br />
        <v-card-text>
          <v-text-field placeholder="name of the API" label='name' v-model="addEditDialog.name" />
          <v-text-field placeholder="host of the API" label='host' v-model="addEditDialog.host" />
          <v-text-field placeholder="PORT" label='port' v-model="addEditDialog.port" />
          <v-text-field placeholder="route of the healthcheck" label='healthcheckRoute'
    ...
    <v-dialog v-model="removeDialog.active" max-width="600">
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="green darken-1" text @click="confirmRemoveAPI">
            OK
          </v-btn>
...
export default {
  ...
  methods: {
    reload() {
      ...
      client.get("/apis?limit=100&offset=0").then((response) => {
    ...
    confirmRemoveAPI() {
      ...
      client.delete("/api/" + this.removeDialog.toRemoveAPI.id)
        .then((response) => {
    ...
    addOrEdit() {
      ...
      if (this.addEditDialog.mode === 'edit') {
        client.put("/api/" + this.addEditDialog.id, {
          name: this.addEditDialog.name,
          host: this.addEditDialog.host,
          port: this.addEditDialog.port,
          healthcheck_route: this.addEditDialog.healthcheckRoute
